...
top_10_member <- all_station_rank %>% 
  mutate(member_trips = member.x + member.y) %>% 
  select(station_name, member_trips) %>%
  arrange(-member_trips) %>% 
  slice(2:11)

gt(top_10_member)

#Top 10 Casual Stations
top_10_casual <- all_station_rank %>% 
  mutate(casual_trips = casual.x + casual.y) %>% 
  select(station_name, casual_trips) %>% 
  arrange(-casual_trips) %>% 
  slice(2:11)
...
duration_table <- valid_rides %>%
  group_by(member_casual) %>% 
  summarize(q1 = quantile(ride_length, 0.25),
...
```{r}
rideable_table <- table(valid_rides$rideable_type, valid_rides$member_casual)
rideable_table
