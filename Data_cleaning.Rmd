...
What if these rides do not start or end at a station, because they are not required to? A search of available information finds that in 2020, Divvy began [introducing dockless bikes as part of their system](https://divvybikes.com/explore-chicago/expansion-temp). This would explain why the latitude and longitude are rounded -- to anonymize user data. For the purpose of this analysis, we will assume that rides with null values in the station name and id fields were dockless rides.
...
valid_rides <- all_rides %>% 
  filter(ride_length >= 60) %>% #Rides < 60s are not valid trips (per Divvy)
  filter(ride_length < (24*60*60)) %>% #Rides > 24h are lost/stolen
...
```{r}
# Subset the valid_rides data frame to exclude rows where start_station_name or end_station_name contains "test"
valid_rides <- valid_rides[!(grepl("test", valid_rides$start_station_name, ignore.case = TRUE) | 
                             grepl("test", valid_rides$end_station_name, ignore.case = TRUE)),]
...
saveRDS(valid_rides, "Data/valid_rides.RDS")
```